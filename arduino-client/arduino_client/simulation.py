...
def create_wokwi_project(
    ...
    if not hex_path:
        raise FileNotFoundError(
            f"未在 {build_path} 找到 .hex，请先编译工程（如菜单 4 编译工程）"
        )
    ...
    diagram = {
        ...
        "parts": [
            ...
            {"id": "led1", "type": "wokwi-led", "left": 280, "top": 80, "attrs": {"color": "green"}},
            {"id": "r1", "type": "wokwi-resistor", "left": 220, "top": 80, "attrs": {"value": "220"}},
        ],
        "connections": [
            ["uno:13", "r1:1", "green", []],
            ["r1:2", "led1:A", "green", []],
...
def run_wokwi_cli(
    ...
    if not os.environ.get("WOKWI_CLI_TOKEN"):
        return False, "未设置 WOKWI_CLI_TOKEN。请在 https://wokwi.com/dashboard/ci 创建 token 并设置环境变量。"
    ...
    try:
        if not subprocess.run(
            ...
            return False, "未找到 wokwi-cli，请安装: npm install -g wokwi-cli"
    except FileNotFoundError:
        return False, "未找到 wokwi-cli，请安装: npm install -g wokwi-cli"
    
    project_dir = Path(project_dir).resolve()
    if not (project_dir / "diagram.json").exists() or not (project_dir / "wokwi.toml").exists():
        return False, "工程目录缺少 diagram.json 或 wokwi.toml，请先对工程执行「仿真运行」生成 Wokwi 项目。"
    ...
    try:
        ...
        return result.returncode == 0, out or "（无输出）"
    except subprocess.TimeoutExpired:
        return False, "wokwi-cli 运行超时"
